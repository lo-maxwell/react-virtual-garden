...
const GardenComponent = () => {
  ...
  const GetPlotAction = useCallback((plot: Plot, selected: InventoryItem | Tool | null) => {
    ...
    if (selected instanceof Tool) {
      if (
        selected.itemData.type == ToolTypes.SHOVEL.name &&
        plot.getItemSubtype() == ItemSubtypes.PLANT.name
      ) {
  ...
  const generatePlots = useCallback((plots: Plot[][]) => {
    return (
      <>
        <div
          className="flex flex-col flex-wrap max-w-[100%]"
          data-testid="garden-plots"
        >
              ...
              {row.map((plot, colIndex) => {
                ...
                return (
                  <PlotComponent
                    key={index}
                    ref={el => {
                      plotRefs.current[rowIndex][colIndex] = el!;
                    }}
                    plot={plot}
  ...
  const plantAll = useCallback(async () => {
    if (
      ...
      selectedItem.itemData.subtype != ItemSubtypes.SEED.name
    )
      return;
    ...
    for (const row of plotRefs.current) {
      for (const plotRef of row) {
        ...
        if (
          plotRef &&
          plotRef.plot.getItemSubtype() === ItemSubtypes.GROUND.name
        ) {
          const plantSeedAction = plantSeed(
            selectedItem,
            plotRef.plot
          ).uiHelper;
  ...
  const harvestAll = useCallback(async () => {
    ...
    plotRefs.current.forEach(row => {
      row.forEach(plotRef => {
        if (
          plotRef &&
          plotRef.plot.getItemSubtype() === ItemSubtypes.PLANT.name
        ) {
          const harvestPlantAction = clickPlant(
            plotRef.plot,
            instantGrow
          ).uiHelper;
          // Performs local update
  ...
  const pickupAll = useCallback(async () => {
    ...
    plotRefs.current.forEach(row => {
      row.forEach(plotRef => {
        if (
          plotRef &&
          plotRef.plot.getItemSubtype() === ItemSubtypes.DECORATION.name
        ) {
  ...
  const addColumn = useCallback(async () => {
    ...
    if (localResult) {
      setGardenForceRefreshKey(
        gardenForceRefreshKey => gardenForceRefreshKey + 1
      );
  ...
  return (
    <>
      ...
      <div className="my-1">
        ...
        >{`${
          !showExpansionOptions ? `Show` : `Hide`
        } Garden Expansion Options`}</button>
        <div
          className={`${showExpansionOptions ? `` : `hidden`} flex flex-row`}
        >
          <div className="flex flex-col">
            <GardenExpansionTooltip row={true} expand={false}>
              <button
                onClick={removeRow}
                disabled={enableGardenExpansionButton(true, false)}
                className={`bg-gray-300 px-4 py-1 mx-1 my-1 text-sm text-purple-600 font-semibold rounded-full border border-purple-200 hover:text-white hover:bg-purple-600 hover:border-transparent focus:outline-none focus:ring-2 focus:ring-purple-600 focus:ring-offset-2`}
                data-testid="shrink-col"
              >
                Remove Garden Row
              </button>
            ...
            <GardenExpansionTooltip row={false} expand={false}>
              <button
                ...
                disabled={enableGardenExpansionButton(false, false)}
                className={`bg-gray-300 px-4 py-1 mx-1 my-1 text-sm text-purple-600 font-semibold rounded-full border border-purple-200 hover:text-white hover:bg-purple-600 hover:border-transparent focus:outline-none focus:ring-2 focus:ring-purple-600 focus:ring-offset-2`}
                data-testid="shrink-row"
              >
                Remove Garden Column
              </button>
            </GardenExpansionTooltip>
          </div>
          <div className="flex flex-col">
            <GardenExpansionTooltip row={true} expand={true}>
              <button
                onClick={addRow}
                disabled={enableGardenExpansionButton(true, true)}
                className={`bg-gray-300 px-4 py-1 mx-1 my-1 text-sm text-purple-600 font-semibold rounded-full border border-purple-200 hover:text-white hover:bg-purple-600 hover:border-transparent focus:outline-none focus:ring-2 focus:ring-purple-600 focus:ring-offset-2`}
                data-testid="expand-col"
              >
                Add Garden Row
              </button>
            </GardenExpansionTooltip>
            <GardenExpansionTooltip row={false} expand={true}>
              <button
                onClick={addColumn}
                disabled={enableGardenExpansionButton(false, true)}
                className={`bg-gray-300 px-4 py-1 mx-1 my-1 text-sm text-purple-600 font-semibold rounded-full border border-purple-200 hover:text-white hover:bg-purple-600 hover:border-transparent focus:outline-none focus:ring-2 focus:ring-purple-600 focus:ring-offset-2`}

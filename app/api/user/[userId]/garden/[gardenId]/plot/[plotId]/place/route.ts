import { placeDecoration } from "@/backend/services/garden/plot/plotService";
...
export async function PATCH(request: Request, { params }: { params: { userId: string, gardenId: string, plotId: string } }) {
	...
	try {
		...
		const result = await placeDecoration(gardenId, plotId, inventoryId, inventoryItemIdentifier, firebaseUid);

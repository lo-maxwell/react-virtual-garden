...
interface ConfirmPlantAllPopupWindowProps {
  showWindow: boolean;
  setShowWindow: (show: boolean) => void;
  plantName?: string;
  onConfirmPlantAll: () => void;
}

export function ConfirmPlantAllPopupWindow({
  showWindow,
  setShowWindow,
  plantName = "this plant",
  onConfirmPlantAll,
}: ConfirmPlantAllPopupWindowProps) {
  const { setConfirmPlantAll } = useAccount();
  const [dontAskAgain, setDontAskAgain] = useState(false);

  const handleConfirmPlantAll = () => {
    // If user checked "don't ask again", disable the confirmation setting
    if (dontAskAgain) {
      setConfirmPlantAll(false);
    }
    ...
    <PopupWindow showWindow={showWindow} setShowWindow={setShowWindow}>
      <div className="w-max bg-reno-sand-200 text-black p-8 rounded-lg shadow-md justify-between items-center">
        <div className="text-2xl font-semibold mb-4 text-center">
          Plant All Confirmation
        </div>
        <div className="text-xl mb-6 text-center">
          Are you sure you want to plant <span className="font-semibold">{plantName}</span>?
        </div>
